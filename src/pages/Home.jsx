...
function Home() {
  ...
  return (
    <>
      ...
      {/* Hero Section */}
      <section className="relative min-h-[calc(100vh-80px)] pt-[144px] px-4 bg-white dark:bg-dark-900 overflow-hidden">
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(59,130,246,0.06)_1px,transparent_1px),linear-gradient(rgba(59,130,246,0.06)_1px,transparent_1px)] bg-[size:72px_72px] bg-[position:0_-4px]" />
        ...
        <div className="max-w-6xl mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-8 pb-20">
          <div className="text-left mt-10 lg:mt-[72px]">
          ...
          <div className="flex justify-center lg:justify-end">
            <div className="relative w-[300px] h-[300px] sm:w-[360px] sm:h-[360px] md:w-[420px] md:h-[420px] rounded-full overflow-hidden group shadow-[0_0_40px_rgba(236,72,153,0.2)] border-[8px] border-white dark:border-dark-800">
              <img
                src={avatar}
                alt="Avatar"
                className="w-full h-full object-cover rounded-full transform group-hover:scale-105 transition-transform duration-700 ease-out"
              />
            </div>
          </div>
        </div>
        ...
        <div className="absolute bottom-6 sm:bottom-8 left-1/2 transform -translate-x-1/2 z-10">
          ...
          >
            <div className="flex flex-col items-center text-gray-700 dark:text-gray-300">
              ...
              />
              <FaChevronDown
                className="w-5 h-5 -mt-1 animate-bounce opacity-70"
                style={{ animationDelay: "150ms" }}
              />
              <FaChevronDown
                className="w-5 h-5 -mt-1 animate-bounce opacity-40"
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            {featuredBlogPost && (
              <article
                onClick={() =>
                  handleNavigate(`/blog/${slugify(featuredBlogPost.title)}`)
                }
                ...
                <div className="p-7">
                  <div className="flex items-center gap-4 mb-3 text-xs text-gray-500 dark:text-gray-400">
                    <span className="inline-flex items-center gap-1">
                      <FaCalendarAlt />
                      {formatDate(featuredBlogPost.date)}
                    </span>
                    <span className="inline-flex items-center gap-1">
                      <FaClock />
                      {estimateReadingTime(
              ...
              <div className="space-y-3">
                {randomPosts.slice(1, 6).map((post, index) => (
                  <article
                    ...
                    onClick={() =>
                      handleNavigate(`/blog/${slugify(post.title)}`)
                    }
              ...
              <div className="rounded-3xl border border-gray-200 dark:border-dark-700 bg-white/90 dark:bg-dark-900/70 p-6 shadow-xl backdrop-blur">
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">
                  {t?.contact?.availability?.label ||
                    (language === "vi" ? "Gọi cho tôi" : "Call me")}
                </h3>
                <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
                  {t?.contact?.availability?.time ||
                    (language === "vi"
                      ? "Làm việc từ 8:00 sáng đến 5:00 chiều"
              ...
              <div className="rounded-3xl border border-gray-200 dark:border-dark-700 bg-white/90 dark:bg-dark-900/70 p-6 shadow-xl backdrop-blur">
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-3">
                  {t?.contact?.chat?.label ||
                    (language === "vi" ? "Trò chuyện với tôi" : "Chat with me")}
                </h3>
                <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
                  {t?.contact?.chat?.message ||
                    (language === "vi"
                      ? "Gửi tin nhắn cho tôi qua mạng xã hội"
                      : "Send me a message on social media")}
                </p>
                <div className="space-y-3">
                  {Object.values(t?.contact?.chat?.options || {}).map(
                    (opt, idx) => {
                      const Icon =
                        opt.icon === "FaSkype"
                          ? FaSkype
                          : opt.icon === "FaFacebookMessenger"
                            ? FaFacebookMessenger
                            : FaEnvelope;
                      return (
                        <div
